...
module.exports = {
    ...
    async execute(interaction) {
        try {
            ...
            // Handle only channel selection interactions
            if (customId.startsWith('select_')) {
                ...
                switch (customId) {
                    ...
                    case 'select_ticket_channel':
                        ...
                        break;
                    default:
                        return interaction.reply({ content: 'Geçersiz kanal seçimi.', ephemeral: true });
                }
                ...
                if (settingType) {
                    ...
                    await interaction.reply({
                        content: `**${settingType === 'game_channel' ? 'Oyun Kanalı' : settingType === 'fun_channel' ? 'Eğlence Kanalı' : settingType === 'cekilis_channel' ? 'Çekiliş Kanalı' : settingType === 'info_channel' ? 'Bilgi Kanalı' : settingType === 'levels_channel' ? 'Level Kanalı' : settingType === 'moderation_channel' ? 'Moderasyon Kanalı' : settingType === 'poll_channel' ? 'Anket Kanalı' : settingType === 'reactor_channel' ? 'Tepki Kanalı' : settingType === 'ticket_channel' ? 'Ticket Kanalı' : 'Araçlar Kanalı'}** başarıyla ayarlandı!`,
                        ephemeral: true
                    });
                }
            }
            ...
            await interaction.reply({ content: 'Bir hata oluştu veya bu seçenekleri kullanmak için belirli izinlere sahip değilsiniz!', ephemeral: true });
        }
    },
};
